...
│  ${green}23.${plain} OpenVPN Management                        │
│────────────────────────────────────────────────│
...
"
    ...
    clear
    echo -e "
  ${green}OpenVPN Management${plain}
  ${green}1.${plain}  List Configurations
  ${green}2.${plain}  Start Configuration
  ${green}3.${plain}  Stop Configuration
  ${green}4.${plain}  View Status
  ${green}5.${plain}  View IP Address
  ${green}6.${plain}  Edit Configuration
  ${green}0.${plain}  Back to Main Menu
  "
    read -rp "Please enter a number [0-6]: " num
    case "${num}" in
        0) show_menu ;;
        1)
            echo "Available OpenVPN configurations:"
            /usr/local/x-ui/bin/openvpn_web.sh list
            ;;
        2)
            echo "Available configurations:"
            /usr/local/x-ui/bin/openvpn_web.sh list
            read -rp "Enter configuration name to start: " config
            /usr/local/x-ui/bin/openvpn_service.sh start "$config"
            ;;
        3)
            echo "Running configurations:"
            while IFS= read -r config; do
                if [ -n "$config" ]; then
                    status=$(/usr/local/x-ui/bin/openvpn_service.sh status "$config")
                    if [ "$status" = "running" ]; then
                        echo "$config"
                    fi
                fi
            done < <(/usr/local/x-ui/bin/openvpn_service.sh list 2>/dev/null)
            read -rp "Enter configuration name to stop: " config
            /usr/local/x-ui/bin/openvpn_service.sh stop "$config"
            ;;
        4)
            echo "Configuration statuses:"
            while IFS= read -r config; do
                if [ -n "$config" ]; then
                    status=$(/usr/local/x-ui/bin/openvpn_service.sh status "$config")
                    echo "$config: $status"
        ...
        5)
            ...
            while IFS= read -r config; do
                if [ -n "$config" ]; then
                    status=$(/usr/local/x-ui/bin/openvpn_service.sh status "$config")
                    if [ "$status" = "running" ]; then
                        ip=$(/usr/local/x-ui/bin/openvpn_service.sh ip "$config")
                        echo "$config: $ip"
                    fi
                fi
            done < <(/usr/local/x-ui/bin/openvpn_service.sh list 2>/dev/null)
        ...
        6)
            echo "Available configurations:"
            /usr/local/x-ui/bin/openvpn_service.sh list
            read -rp "Enter configuration name to edit: " config
            if [ -f "/usr/local/x-ui/openvpn/$config.ovpn" ]; then
                ${EDITOR:-nano} "/usr/local/x-ui/openvpn/$config.ovpn"
            else
                echo "Configuration file not found: $config.ovpn"
            fi
            ;;
        *)
            LOGE "Please enter a correct number [0-6]"
            ;;
    esac
